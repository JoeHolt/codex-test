...
class Game {
  constructor() {
    ...
    this.effects = ['Nothing', 'Super Knight', 'Free Pawn'];
  ...
  nextEffect() {
    const idx = Math.floor(Math.random() * this.effects.length);
    this.currentEffect = this.effects[idx];
  }
  ...
  updateStatus() {
    ...
    document.getElementById('effect').textContent = `Effect: ${this.currentEffect}`;
  }
}
